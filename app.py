...
with tab_glosas:
    ...
    if st.session_state.glosas_ready and st.session_state.glosas_data is not None:
        ...
        # ============================================================
        # OTIMIZAÇÃO — Tratamento antecipado do botão FECHAR DETALHES
        # (isso evita rerenderização pesada antes do rerun)
        # ============================================================
        if st.session_state.get("close_details_clicked"):
        ...
        # ============================================================
        # === DETALHES DO ITEM SELECIONADO (OTIMIZADO) ===============
        # ============================================================
        if selected_item_name:
            st.markdown("---")
            st.markdown(f"#### 🔎 Detalhes — {selected_item_name}")
        
            # Agora o botão apenas marca um flag e NÃO chama st.rerun() aqui
            if st.button("❌ Fechar detalhes", key="btn_fechar_detalhes_item"):
                st.session_state["close_details_clicked"] = True
        
            # Renderização preguiçosa para evitar custo quando o bloco está fechado
