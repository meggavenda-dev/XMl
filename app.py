...
def _download_excel_button(df_resumo: pd.DataFrame, df_agg: pd.DataFrame, df_auditoria: pd.DataFrame, label: str):
    ...
    Abas: Resumo por arquivo | Agregado por lote | Auditoria
    """
    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        # Formatação de moeda
        def format_currency_sheet(ws, header_row=1, currency_cols=("valor_total", "total_tag", "subtotal_itens_proc", "subtotal_itens_outras", "subtotal_itens")):
        ...
        if ws is not None:
            format_currency_sheet(ws, currency_cols=("valor_total",))
        # Auditoria (só formatará se as colunas existirem)
...
with tab1:
    ...
    if files:
        ...
        if resultados:
            ...
            df_disp = _df_display_currency(df, ['valor_total'])  # cópia para exibir R$
